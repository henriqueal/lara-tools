...
//			return 'attributes.set({ function:{ MajorityVoter:{ delay:"20.167"}}});';
	...
	trce: function(args){
                ...
                with(tools.xilinx){
                        ...
                        if(args.trceOptions != undefined) {
                                if( tools.xilinx.trceOptions == null)
                                        tools.xilinx.trceOptions = new java.util.HashMap();
                                else tools.xilinx.trceOptions.clear();

                                for(var __opt__ in args.trceOptions)
                                        trceOptions.put(__opt__, args.trceOptions[__opt__]);

                                rt.setTrceOptions(trceOptions);
                        }

                        rt.trce(design, rt.ucfFile); // ucFile can be null
//                      return 'attributes.set({ function:{ MajorityVoter:{ msgInfos:"0"}}});';
			var __js__ = ""+(new org.reflect.xilinx.Report()).getTRCEReportStr(
                                        rt.workingFolderStr, "design", design);
                        return __js__;
